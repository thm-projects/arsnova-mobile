 ...
 */
Ext.define('ARSnova.view.speaker.SessionInfoPanel', {
	extend: 'Ext.Carousel',
	...
	initialize: function () {
		...
		this.toolbar = Ext.create('ARSnova.view.components.QuestionToolbar', {
			...
			backButtonHandler: function (animation) {
				...
				if (sTP.sessionEditPanel.inclassBackButtonHandle) {
					sTP.animateActiveItem(sTP.inClassPanel, animation);
					sTP.sessionEditPanel.inclassBackButtonHandle = false;
				} else {
	...
	getAllSkillQuestions: function () {
		...
		this.getController().getQuestions(sessionStorage.getItem("keyword"), {
			success: function (response) {
				...
				var panel = ARSnova.app.mainTabPanel.tabPanel.speakerTabPanel.sessionEditPanel;
